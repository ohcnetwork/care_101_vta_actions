...
**Newline handling in shell feedback strings:**

When a shell variable holding a newline sequence (e.g. `\n\n`) is interpolated as `%s` in `printf`, bash double-quoted strings (`"..."`) pass the literal characters `\n\n` — they are not expanded. This causes the newline to appear as raw text in the rendered feedback.

Use `$'...'` (ANSI-C quoting) to embed real newlines in variables:
...
Flag any feedback variable set with double quotes that relies on `\n` for formatting.
